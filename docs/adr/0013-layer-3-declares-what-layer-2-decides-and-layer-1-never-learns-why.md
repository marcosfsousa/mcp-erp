...
**No answer carries the identifier it is about, and a caller maps answers to items by position.** That is not a convenience: an answer that named its row would make `not_found` on a foreign row distinguishable from `not_found` on a row that never existed, which is the existence oracle [ADR-0002](0002-refusal-shape-follows-the-remedy.md) declined to ship, arriving through the fold instead of through the refusal. Position costs the caller nothing — they wrote the list.
