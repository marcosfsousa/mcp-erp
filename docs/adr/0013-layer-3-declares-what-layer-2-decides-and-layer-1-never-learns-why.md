...
- **Amended:** 2026-08-21 — substantive, by [#82](https://github.com/marcosfsousa/mcp-erp/issues/82), which found layer 1 naming a cause it had not established. **The signal for an argument a declaration forbids is `UnusableArgument`, declared in layer 2**, where it was `ValueError`. The reason given for `ValueError` was about the *raise* and the defect was at the *catch*: layer 1 wraps a handler's whole iteration, the store is awaited inside it, and a type the standard library shares with every library below made any failure down there answer *the arguments are not ones the declared schema permits*. `tests/wire/` additionally holds **three assertions that are not over HTTP**, two of which could not be. See *Handlers in layer 3, adapters in layer 1* and *Test directories, named for artifacts*. This reverses the choice of exception type and nothing else — the `-32602`, the absent `Reason` and *layers 1 and 3 import nothing from each other* all stand.
- **Amended:** 2026-08-21 — additive, by [#85](https://github.com/marcosfsousa/mcp-erp/issues/85), which wrote the falsifiers three green suites were read as already having. **No decision changes and no row is added; two counts were wrong and are corrected.** `tests/wire/` holds ~~one~~ ~~two~~ **four assertions that are not over HTTP, in three kinds** — the fold's property with no altitude, an effect with no tool, and a precondition on the instrument that has been there since [#37](https://github.com/marcosfsousa/mcp-erp/issues/37) uncounted. With the **three** the amendment above adds the same day, the running total is **seven**. *Authorization code flow* carries a **second diagnosis**, because the realm's refusals for the client that has no file are now asserted and only that job can reach them. See *Test directories, named for artifacts* and *Continuous-integration jobs, one per seam*. No decision here is reversed.

- **Amended:** 2026-08-21 — substantive, by [#109](https://github.com/marcosfsousa/mcp-erp/issues/109), which found layer 1 closing a disclosure and closing the only record of it in the same act. **Containment at dispatch is per callback, where it was per iteration** — `_render`, `_fold`, `_result` and the whole of `on_list_tools` sat outside #82's guard, so *both eras answer identically* was true of the handler's iteration and false of everything after it, and a handler yielding a `Decimal` reached a legacy caller as `Object of type Decimal is not JSON serializable`. And **a failed key-set fetch is logged rather than suppressed**, on the same reasoning #82 gave for logging at dispatch: without it, closing the disclosure closes the only record that anything went wrong. `tests/wire/` additionally holds **two assertions that are not over HTTP**, bringing the running total to **nine**; the tests that close the containment gap rescope #82's third claim rather than adding one, and the enumeration behind that total now lives in `tests/wire/README.md` rather than being an increment three documents apply by hand. See *Handlers in layer 3, adapters in layer 1* and *Test directories, named for artifacts*. This reverses the span of the catch and nothing else — the `-32602`, the `UnusableArgument` type, the absent `Reason` and *layer 1 never learns why* all stand.
...
*Amended 2026-08-21 by [#82](https://github.com/marcosfsousa/mcp-erp/issues/82), which found the catch and the sentence describing it disagreeing.* **The signal is `UnusableArgument`, and it is declared in layer 2.**

The struck clause argued the *raise* and the defect was at the *catch*. Both halves of the sentence are true of `ValueError` on their own and false together: layer 1 wraps `async for outcome in registration.handler(...)`, which is where the store is awaited, so *catches it around the handler's own iteration* and *nowhere else* were two different scopes written as one. Any `ValueError` from below the handler's argument parsing — a driver, a conversion, anything in the read path — became `-32602` with `str(...)` as the message: a failure of ours reported to the caller as a mistake of theirs, carrying whatever the thing that failed happened to say.

**The span could not be narrowed instead**, and that is what settles the shape rather than taste. An async generator's body does not run until it is iterated, so there is no earlier moment at which layer 1 could wrap the parsing alone; splitting each handler into a part that can raise and a part that cannot would leave the same catch around the same iteration for whatever slipped past the split. Narrowing the *type* is the only edit that makes the comment true.

**Layer 2 rather than layer 3**, which is the *Layer boundaries* contract read literally: `mcp_erp.transport` and `mcp_erp.purchase_to_pay` are independent, so a name in the domain package is one layer 1 could not catch and a name layer 1 owned is one a handler could not raise. The struck clause's own words are *without importing anything **layer 1** owns*, and layer 2 satisfies that — `Action`, `Decision` and `Principal` cross the same way. It costs one class in `authorization/arguments.py` that decides nothing and survives ejection with nothing left raising it, which is what this seam looks like from the other side whenever the domain is deleted.

**Nothing else moves.** It still answers `-32602`, still carries no `Reason`, is still not a fourth denial class, and layer 1 still never inspects the message. What changes is that the catch now means what the paragraph above it claims, and everything else out of a handler's iteration fails loudly as ours.

*Amended 2026-08-21 by [#109](https://github.com/marcosfsousa/mcp-erp/issues/109), which found the other half of the same sentence short.* **Containment is per callback, not per iteration.**

The amendment above narrowed the *type* one catch takes and left the *span* of the catch beside it where #82 found it: around `async for outcome in registration.handler(...)`, and nowhere else. That span is the one thing about it that was never argued — it is where the `UnusableArgument` catch has to be, and the general one was written next to it because that is where the code already was. Everything after the iteration was therefore uncontained: `_render`, `_fold` and `_result`, and the whole of `on_list_tools`, which had no catch of any kind.

**What that leaves is a mechanism rather than a live disclosure, and the distinction is worth keeping straight.** No path through the residual carries an infrastructure fact in this codebase today — `requisition.py` stringifies its amounts before layer 1 sees them, and the listing reaches no store. What was false is the *scope* of the claim `INTERNAL_FAILURE` makes: *both eras answer identically* held for the iteration and for nothing after it, and a handler yielding a `Decimal` answered a legacy caller `code: 0` with `Object of type Decimal is not JSON serializable` while a modern one got `-32603` and a generic message. ADR-0009 is why the legacy leg counts: not built is not unreachable.

**Each callback is one boundary now**, with the body factored out below it so the boundary is the whole of what the callback is, and an `MCPError` re-raised untouched — an `MCPError` is this module having already chosen what to say, and re-wrapping one would replace a `-31010` carrying a `Reason` with a generic failure. The `UnusableArgument` catch stays exactly where it was, because it is the one catch here that is about a type rather than about a span.

**The same sentence is applied one gate lower, in `KeySet._refetch`.** That block catches every `Exception` a fetch can raise and turned each into `401 unknown_key` with no record anywhere — so a defect of ours was indistinguishable from an authorization server being down *and* invisible, refusing every caller and retrying silently once per cooldown for as long as the process ran. It logs now, on this document's own reasoning about the dispatch catch: closing the disclosure must not close the only record that anything went wrong. **Where that record goes was established rather than assumed** — uvicorn configures no root handler, so it reaches `stderr` through `logging.lastResort` and Compose collects the container's `stderr`; `server/README.md` §*Where a failure of ours is recorded* carries the chain and states what the missing timestamp costs. **The caught set is not reopened**, and neither is the cancellation exemption: `CancelledError` descends from `BaseException`, a torn-down request still tears down, and #109 wrote that falsifier *before* the rewrite rather than after it.
...
*Amended 2026-08-21 by [#82](https://github.com/marcosfsousa/mcp-erp/issues/82).* **Three more assertions in `tests/wire/` are not over HTTP, and two of them could not be.**

The three refusals #82 corrected are all things the server does regardless of the caller, which is this directory's own dividing line, so none of them is a `(principal × tool × resource)` row. Two additionally have **no altitude to be provoked at**: a real authorization server does not serve a key set PyJWT cannot read, and a real store does not raise the exception layer 1 was catching. Driving them means constructing the failure in process — a mock transport under a real client, and a handler that fails where a store would have been awaited — so `test_the_cause_a_refusal_names.py` sits beside the file the amendment above put here, on the same reasoning and against the same alternative.

**No scenario row was minted for any of the three.** ADR-0010 makes membership one row per clause this project enforces, each recording the removal that makes it pass, and nothing gets through here: the caller is refused either way, and what changed is which word the refusal carries. That is the refusal the two amendments above already take, applied to a defect rather than to a feature.

*Amended 2026-08-21 by [#85](https://github.com/marcosfsousa/mcp-erp/issues/85).* **Two more assertions in `tests/wire/` are not over HTTP, and they are two further kinds of exception.** ~~One assertion~~ ~~Two~~ **Four**, in **three kinds** — counting the lineage as it stood before the three above.

`test_two_simultaneous_decisions_on_one_requisition_mint_one_order` reads `purchase_order` directly, through `tests/fixtures.py`. *One order was minted* is the half of ADR-0002's promise no response can carry: a refused item answers with a reason and no order, so the wire cannot distinguish *no second order* from *a second order that was not shown* — and ADR-0002's count of five cut every read tool that demonstrated no authorization behaviour, so no tool lists one. #41's exception above is a claim with **no altitude**; this is a claim with an altitude and no *tool*. The assertion is about what a call did to the rows behind it, and those rows are reachable only one layer down.

`test_the_gateway_runs_a_single_worker` is the other two, and they arrived with [#37](https://github.com/marcosfsousa/mcp-erp/issues/37) rather than with this ticket. They read `gateway/nginx.conf` and `compose.yaml` off the checkout, and they are a **precondition on the instrument** rather than a claim about the subject at all: `worker_processes 1` is what makes the round-robin assertion beside it measure affinity instead of merely staying green, and nginx answers no request that reports its worker count. `docker compose exec gateway nginx -T` would, and would put a docker invocation inside a suite that otherwise speaks only HTTP — so the committed file is read, on the same terms as the seed's rendering checks.

**That test declared its own exception in its docstring, and this document never counted it.** So the count is corrected twice in one amendment, and the second correction is the one worth recording: an enumeration maintained only when somebody writes a *new* exception cannot see one that was already sitting there. The first two were found by writing them; the third kind was found by counting, which is the check this ticket is about applied to this ticket's own documents.

So the narrowing holds and the count moves: the directory is still named for the altitude its assertions share, and the exceptions are now ~~two~~ **three kinds over four assertions** — one property that has no altitude, one effect that has no tool, and one precondition on the instrument the other assertions are read through. ADR-0003 already priced the credential the second needs, choosing a suite that holds a database password over a test-only reset route, and `tests/attack_suite/` has read the same table on those terms since #41. Counted together with the three the amendment above adds the same day, `tests/wire/` holds ~~**seven**~~ **nine** assertions that are not over HTTP — see the amendment below, which raised it and stopped it being a number.

*Amended 2026-08-21 by [#109](https://github.com/marcosfsousa/mcp-erp/issues/109).* **Two more are not over HTTP, and neither could be. ~~Seven~~ Nine — and the list, not the number, is now what this claim is.**

Both are about `KeySet._refetch` and both sit in `test_the_cause_a_refusal_names.py`, beside the key-set assertion #82 put there and for the same reason: no caller can make a real authorization server's fetch fail in a chosen way. The first is that **a failure inside the fetch leaves a record naming it** — driven with `caplog`, which is the only altitude a log record has. The second is that **cancellation inside the fetch still tears the request down**, which is load-bearing on the rewrite in the same commit: the obvious widening to `BaseException` would make the record cover everything and keep every other test in the file green.

**Two things #109 also added are second tests of assertions that were already counted.** *The caller learns none of what that record holds* is #82's key-set non-disclosure reached through a different failure. And the tests that close **#82's third claim at its true scope** — *dispatch's catch does not dress our own failure as the caller's*, which was asserted around the handler's iteration and was false of `_render`, `_fold`, `_result` and the whole of `on_list_tools` — are that claim asserted properly rather than a new one. The claim was never scoped to the iteration in words; it was only ever true of it.

**The number stopped being maintainable by increment, so `tests/wire/README.md` now carries the enumeration and this paragraph defers to it.** #85 raised the count twice in one edit because an enumeration maintained only when somebody writes a *new* exception cannot see one already sitting there; #109's own two reviews then read the same running total as seven-plus-two and seven-plus-three, because *assertion* had been used to mean a test function, a claim, and a file read, in three different amendments. The unit is stated there once — **one thing asserted that no request can show** — with a row per item and the ticket that added it. This document keeps the total and stops restating the derivation, which is the same treatment #66 gave the job count one section down.
...
*Amended 2026-08-24 by [#124](https://github.com/marcosfsousa/mcp-erp/issues/124).* ***Server posture* widens to the opt-in TLS profile, and no row is added.** The profile is a second deployment of the same stack, and until this commit no runner had ever selected it: `docker compose --env-file tls.env up` was documented in three files and executed on one machine. The job now creates `keycloak-certificate` with that env file before it brings the default stack up, and removes it again — the profile selected, the image resolved from the build another service does, the container created, its `Cmd` empty. It stops short of minting a certificate and of any handshake: those need the trust step only a human can take, and ADR-0014 already prices that as the profile's cost.

It is a step in this job rather than a row of its own, on the same rule as the two additions above: contexts and job names are held equal in both directions, so a step needs no ruleset edit. The diagnosis does not widen in kind either — *deploys something other than what it should, with no caller's authorization involved* already reads on a profile that cannot be brought up, and there is no caller in it.
...
*Amended 2026-08-20 by [#44](https://github.com/marcosfsousa/mcp-erp/issues/44), the fourth consumer and the last one expected, which settled it.* **The pattern stays unfactored, and the reason is no longer "not this ticket".** Three tickets deferred the question on the ground that none of them would ship a refactor of somebody else's job; that ground runs out here, because there is no fifth ticket to defer to. What decides it is what the copies actually are: **four steps, three of them one line each** — install, a hosts-file line, `docker compose build`, `docker compose up --wait`. Those four are exactly what the enumeration under each job spends its words justifying, and a composite action would replace them with one `uses:` a reader cannot check without opening another file. The comment above each job — the *diagnosis*, which is the part that is never shared — is what has to sit beside the steps, and three copies of fifteen lines of YAML is the cheaper half of that trade. **It reopens on evidence rather than on taste**: a fifth Compose consumer, or a bring-up change that has to be made identically in four places twice. The argument stays where #46 put it, in the `server-posture` header comment, and this paragraph does not restate it.

*Amended 2026-08-20 by [#47](https://github.com/marcosfsousa/mcp-erp/issues/47), which is the last ticket this section was waiting on.* **The set equality above stopped being an instruction and became a check.** `tests/test_required_checks.py` holds three enumerations of this job set equal in both directions — the table above, `ci.yml`'s job `name:` values, and what the `main` ruleset requires — and the third is read from the API on every run rather than from a copy committed beside the other two. A copy would go green on exactly the pull request this document is afraid of: the one that renames a job, edits the copy to match, and leaves the ruleset waiting on a context that no longer arrives. **The table is inside the check** for the reason #66 found the hard way, having fixed a disagreement in both directions at once — a document that enumerates a set and is never compared to it drifts from it, and this one had.

**#47 executed the order this document owns rather than restating it.** The job shipped first and reported on its own pull request — red, because the ruleset required nothing yet, which is the check saying so out loud — and the ruleset followed once all ten contexts had reported. The consequence below is discharged on the merge that puts the tenth job on `main`, and the window between the two is the one thing the order does not remove: until that merge, a branch cut from `main` carries nine of the ten jobs and would sit pending on the tenth. It is closed by merging, not by waiting.

**Force pushes to `main` were already blocked, and #47 is what makes that a standing claim rather than a setting.** The `non_fast_forward` rule has been in this ruleset since it was created on 2026-08-05, so the gap #66 recorded on the way past was closed before it was noticed — checked rather than assumed, which is the half of this that was worth doing. What #47 adds is the assertion: the same suite that holds the contexts equal fails if that rule is ever removed. The gap it covers belongs to *Published documents are immutable* rather than to the required contexts — that job warns and passes with no comparable base commit, having checked nothing, and a force push to `main` is the only reachable path to that green tick.

*What the fourth job runs is not only wire tests.* `tests/attack_suite/` holds the drift check that keeps `scenarios.yaml` and the tests in bijection, and it needs no Compose — it reads the table and the tests' own source. It is collected there because it lives in that directory, which is the same reading this document already takes for `tests/matrix/test_the_matrix_holds_together.py`: a suite that skipped its own invariants when run whole would be a suite with a hole in it.
