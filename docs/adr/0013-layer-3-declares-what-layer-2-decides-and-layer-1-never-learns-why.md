...
This section counted generators where the count is fixed by *renderings*: the seed has three, and the paragraph above named two renderers. The missing one is the ERP's own rows, which speak cost centres, vendors and people — layer 3's words — so it lands in `purchase_to_pay/` beside the fixture generator and is deleted with them. Layer 3 therefore holds **two** generators and layer 2 holds one, which is the same rule read carefully rather than a new one: *the vocabulary decides the layer, and the layer decides what the deletion takes.*
