...
- **Amended:** 2026-08-20 — additive, by [#47](https://github.com/marcosfsousa/mcp-erp/issues/47), which built the last job and then the ruleset, in that order. The **tenth job, *Required checks match the ruleset***, exists, and with it `tests/test_required_checks.py`: the set equality this document requires is now asserted **three ways** — the table here, the workflow, and the live `main` ruleset — rather than held by hand. Every context in the table is **required on `main`**, and **force pushes to `main` are blocked in the same ruleset**, which is what closes the gap *Published documents are immutable* leaves open by construction. See *Continuous-integration jobs, one per seam*. No decision here is reversed.
...
**The order this document owns was executed rather than asserted.** The job shipped first and reported on its own pull request before the ruleset required anything; the ruleset followed once all ten contexts had reported. The consequence below is discharged, not merely respected.

**Force pushes to `main` are blocked in the same ruleset**, which is the second half of what #47 landed and belongs to *Published documents are immutable* rather than to the required contexts. That job warns and passes with no comparable base commit, having checked nothing, and the paragraph above already names the force push as the only reachable path to that green tick. The rule closes it without touching the job — which is what keeps the job's diagnosis one thing.
