...
So the narrowing holds and the count moves: the directory is still named for the altitude its assertions share, and the exceptions are now ~~two~~ **three kinds over four assertions** — one property that has no altitude, one effect that has no tool, and one precondition on the instrument the other assertions are read through. ADR-0003 already priced the credential the second needs, choosing a suite that holds a database password over a test-only reset route, and `tests/attack_suite/` has read the same table on those terms since #41. Counted together with the three the amendment above adds the same day, `tests/wire/` holds ~~**seven**~~ assertions that are not over HTTP.

*Amended 2026-08-21 by [#109](https://github.com/marcosfsousa/mcp-erp/issues/109).* **Two more are not over HTTP, and neither could be. ~~Seven~~ Nine.**

Both are about `KeySet._refetch` and both sit in `test_the_cause_a_refusal_names.py`, beside the key-set assertion #82 put there and for the same reason: no caller can make a real authorization server's fetch fail in a chosen way. The first is that **a failure inside the fetch leaves a record naming it and the caller learns none of what that record holds** — one claim asserted in two directions, because a fix satisfying either alone is the wrong fix, and driven with `caplog`, which is the only altitude a log record has. The second is that **cancellation inside the fetch still tears the request down**, which is load-bearing on the rewrite in the same commit: the obvious widening to `BaseException` would make the record cover everything and keep every other test in the file green.

**#109's third claim is #82's third, rescoped rather than added, and that is why the count moves by two.** *Dispatch's catch does not dress our own failure as the caller's* was asserted around the handler's iteration and was false of everything after it. What closes it — a `Decimal` outcome through `_render` and through `_fold`, a failure inside the listing, the two eras' answers held equal at each, and a control that the listing still answers — are new tests of an existing claim, at the altitude that claim already sat at. Counting them again would be this document doing the thing #85 caught it not doing: keeping an enumeration by increments rather than by reading what is there.
