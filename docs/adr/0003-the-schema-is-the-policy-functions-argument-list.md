...
- **Amended:** 2026-08-20 — additive, by [#84](https://github.com/marcosfsousa/mcp-erp/issues/84), which fixed the identifier mint's pad. **One suite wipes between tests rather than once**, and it is the one whose subject is the mint's own high-water mark — the reason given below for wiping once is that no row can disturb another, and the high-water mark is the one piece of state every row moves and none owns. See *The organisation is authored; the test data is generated*. The cadence for every other suite is unchanged and no decision here is reversed.
- **Amended:** 2026-08-20 — substantive, by [#43](https://github.com/marcosfsousa/mcp-erp/issues/43), which built the matrix. A fixture's identifier is an **ordinal, not the row's name**, because two rules stated here collide and the legibility deviation wins; and the row count stops being written down, on this trail's own reasoning about counts. See *The organisation is authored; the test data is generated* and *The matrix skeleton*. No decision here is reversed.
...
Each write row owns a fixture outright, so no row can disturb another, and the seeder wipes and reloads once before a full run rather than between rows. *(Amended 2026-08-20 by [#84](https://github.com/marcosfsousa/mcp-erp/issues/84): **one suite reloads between its own tests**, because the premise stated here does not hold for it. `tests/wire/test_the_identifier_mint.py` drives what each table mints next, which is derived from the highest identifier present — a piece of state no row owns and every row moves, so one of its tests would otherwise set up the next one. Every other suite keeps the cadence above.)* That leaves #8 free to choose an in-process runner or real HTTP later, and it puts no reset route on a server whose entire subject is authorization.
...
**Guardrail.** Each row's `given` block is a flat literal record — cost centre, amount, submitter, state — with no defaults, no inheritance, and no references to other rows. The generator is a loop emitting one requisition per row, with ~~an id derived from the row's own name~~ **an ordinal identifier, and the row's name carried beside it**. The moment a `given` block wants a conditional, it is becoming the DSL constraint #4 refused, and review should catch it. The generated seed is **committed** and continuous integration fails on any diff, so what ships is readable as data rather than knowable only by executing a script.

*Amended 2026-08-20 by [#43](https://github.com/marcosfsousa/mcp-erp/issues/43), which built the generator and found two rules in this document pulling against each other.* A name-shaped identifier — `req_over_threshold` — is **not guessable**, and the *Legible identifiers* passage above takes a `SHOULD` deviation precisely so that `row_probe_indistinguishable` can guess a foreign identifier rather than be handed one. It would additionally read as zero to the mint, which derives the next identifier from the highest trailing integer that exists. So identifiers stay `req_0001` upward in declaration order, and what ties one to the row that owns it is a `row` field on the rendering — which the loader ignores and a reader does not.

**The guardrail's purpose survives intact, because nothing keys on the number.** Every suite in the repository asks for a fixture by the matrix row that owns it, or by the property it needs — a partition, a status, an amount below the threshold. So inserting a row renumbers the rendering, the drift check shows the renumbering, and no assertion moves. What was traded away is reading an identifier and knowing its row; what was bought is the deviation this document already paid for staying true.

**One field left the block and one stayed.** `currency` is absent: it has exactly one legal value, so a column could only restate the schema's own `CHECK`. `cost_centre` is present although a person holds exactly one and `submit_requisition` stamps it — because the fixture generator reads `matrix.yaml` and never the seed, so deriving it would couple the two generators this document keeps apart. The duplication is therefore *by authorship*, which is what makes a drift check the right control for it: `tests/matrix/test_the_matrix_holds_together.py` asserts every fixture's centre against its submitter's.
...
*Amended 2026-08-20 by [#43](https://github.com/marcosfsousa/mcp-erp/issues/43), which wrote the rows.* **The count is not restated here.** It lives in [`matrix.yaml`](../decision-matrix/matrix.yaml)'s `meta` block, re-derived with `yq '.meta' docs/decision-matrix/matrix.yaml` — the same move note `#4` of the map made for `scenarios.yaml`, for the same reason: a derived number kept in two documents drifts, and this trail has caught that four times. The estimate above was made while charting and the shape held; three of the six splits moved, because #66 handed `tools/list` a fifth scope set and because both tools that hydrate a named row earn a row for the identifier no row carries.

**What is a decision rather than a count, and so does belong here:** the table's floor is that **every reason in the union of both layers' declared sets is expected by at least one row**, which is what makes the closed vocabulary reachable rather than merely declared; and its ceiling is soft, crossed by reviewing whether two rows restate one branch rather than by adding a row. Both are asserted, in `tests/matrix/test_the_matrix_holds_together.py` — a bound nothing checks is a bound.
...
- **#12 (module boundaries)** inherits a seam: the seed generator reads the matrix, and the policy function reads neither.
  ...
  *Amended 2026-08-18 by [#35](https://github.com/marcosfsousa/mcp-erp/issues/35), which built them.* **A second becomes a third.** The paragraph above accounts for the fixtures and for identity, and leaves the ERP's own organisation rows — this section's first rendering — with no renderer named. That one speaks cost centres, vendors and people, so it is layer 3's and sits beside the fixture generator: `mcp_erp.purchase_to_pay.organisation`. The rule is untouched and is what produces the answer; only the count moves. [ADR-0013](0013-layer-3-declares-what-layer-2-decides-and-layer-1-never-learns-why.md) carries the reasoning.
