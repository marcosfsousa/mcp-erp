...
Map constraint `#7` already commissions *"one narrative synthesis at the end linking down into"* the decision records; map constraint `#12` enrols **the write-up** as a derived artifact with the declared state *not yet written; joins when it exists*; and [`docs/write-up-notes.md`](../write-up-notes.md) declares itself that write-up's staging file, to be **deleted in the same commit that first renders from it**. The walkthrough *is* that synthesis. Inventing a third document beside it would create two artifacts whose difference nobody could state in six months — the two-sources-one-fact drift `write-up-notes.md` explicitly refuses to become.
