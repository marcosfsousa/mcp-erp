...
def test_the_metadata_document_answers_without_a_token() -> None:
    ...
    # deduplicated: two read tools, one write and one decide contribute three
    # strings. `erp.decide` joined them at #40, with the tool that declares it.
    assert document["scopes_supported"] == ["erp.decide", "erp.read", "erp.write"]
...
def test_the_tool_endpoint_challenges_a_request_with_no_credentials() -> None:
    ...
    assert parameters["scope"] == "erp.decide erp.read erp.write"
