...
declarations and the entity schemas alike, and several suites under
``tests/wire/`` assert it — an argument recorded once beside one of those
assertions would be a claim in a place nobody edits this package from (#112).
...
this stack validates arguments against a published ``inputSchema`` — which
:data:`~mcp_erp.purchase_to_pay.submit_requisition.AMOUNT_PATTERN` states
outright, and is why the handler matches that constant itself rather than
trusting the declaration beside it. So a caller sending a forbidden key is
refused by nothing. What closes that hole is the handler: it never reads the
key, and the write takes ``principal.partition`` regardless. Each declaration
states the shape and the handler holds it, and neither is the whole claim by
itself.
"""
